(ns telecomtrade.operation
  "OperationActor -- one telecom/electronics-equipment-wholesale
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (TelecomTradeAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Telecom
  Supply-Chain Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one telecom/electronics-equipment-wholesale operation
  (intake -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed.
  A telecom-order's life is advanced by MANY ops (intake / sourcing-
  verify / dispatch / settle), each its own independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (trading supervisor). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:delivery/dispatch`/`:invoice/settle` ALWAYS reach this node when the
  governor is clean -- see `telecomtrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [telecomtrade.telecomtradeadvisor :as telecomtradeadvisor]
...
(defn build
  ...
  `telecomtrade.store/Store`).
  opts:
    :advisor      -- a `telecomtrade.telecomtradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (telecomtradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TelecomTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (telecomtradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(telecomtradeadvisor/trace request p)]})))

      ;; Telecom Supply-Chain Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
