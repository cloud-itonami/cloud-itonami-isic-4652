(ns telecomtrade.phase
  "Phase 0->3 staged rollout for the telecom/electronics-equipment-
  wholesale actor.

    Phase 0  read-only        -- no writes, still governor-gated.
    Phase 1  assisted-intake  -- telecom-order intake allowed, every
                                 write needs human approval.
    Phase 2  assisted-verify  -- adds sourcing-verification writes,
                                 still approval.
    Phase 3  supervised auto  -- governor-clean, high-confidence
                                 `:order/intake` (no capital risk / no
                                 supply-chain-sourcing risk yet) may
                                 auto-commit. `:delivery/dispatch`/
                                 `:invoice/settle` NEVER auto-commit, at
                                 any phase.
  ...
  fact, not a rollout milestone still to come. Physically dispatching
  real telecom/networking equipment to a counterparty and settling a
  ...
  human trading supervisor's call. `telecomtrade.governor`'s high-stakes
  ...
  domain has only ONE member (`:order/intake`) -- no separate
  no-capital-risk lifecycle distinct from the telecom-order itself.")

(def read-ops  #{})
(def write-ops #{:order/intake :sourcing/verify :delivery/dispatch :invoice/settle})
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:order/intake :sourcing/verify}                   :auto #{}}
...
(defn verdict->disposition
  "Map a Telecom Supply-Chain Governor verdict to a base disposition
